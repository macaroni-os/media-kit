...
SRC_URI="https://ffmpeg.org/releases/ffmpeg-8.1.1.tar.xz -> ffmpeg-8.1.1.tar.xz"
SLOT="0"
KEYWORDS="*"
...
src_configure() {
	local myconf=(
	  ...
	  --docdir="/usr/share/doc/ffmpeg-8.1.1/html"
	...
	if use sdl ; then
	  myconf+=( --enable-ffplay )
	else
	  myconf+=( --disable-ffplay )
	fi
	if use sdl ; then
	  myconf+=( --enable-sdl2 )
	else
	...
	if use X ; then
	  myconf+=( --enable-libxcb )
	else
	  myconf+=( --disable-libxcb )
	fi
	if use X ; then
	  myconf+=( --enable-libxcb-shm )
	else
	  myconf+=( --disable-libxcb-shm )
	fi
	if use X ; then
	  myconf+=( --enable-libxcb-xfixes )
	else
	  myconf+=( --disable-libxcb-xfixes )
	fi
	if use zlib ; then
	  myconf+=( --enable-zlib )
	else
	...
	if use amr ; then
	  myconf+=( --enable-libopencore-amrwb )
	else
	  myconf+=( --disable-libopencore-amrwb )
	fi
	if use amr ; then
	  myconf+=( --enable-libopencore-amrnb )
	else
	  myconf+=( --disable-libopencore-amrnb )
	fi
	if use codec2 ; then
